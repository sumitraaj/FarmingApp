...
const AvailableMeals = () => {
  ...
  const mealsList = current_data.map((equipment) => (
    <MealItem
      key={equipment._id}
      id={equipment._id}
      name={equipment.Equipmentname.length > 20 ? equipment.Equipmentname.slice(0, 20) : equipment.Equipmentname}
      // description={equipment.Description.trim(2)}
      description={equipment.Description.length > 40 ? equipment.Description.slice(0, 40) : equipment.Description}
      price={10}
    ></MealItem>
  ));
    useEffect(() => {
      fetch("http://34.131.134.106/equipments", {
      method: "POST",
      crossDomain: true,
      headers: {
  ...
  const fetchSearch = (event) => {
    ...
    fetch("http://34.131.134.106/searchE", {
      method: "POST",
      crossDomain: true,
      headers: {
        "Content-Type": "application/json",
  ...
  const fetchPrice = (event) => {
    console.log("yes");
    fetch("http://34.131.134.106/searchP", {
      method: "POST",
      crossDomain: true,
      headers: {
        "Content-Type": "application/json",

...
function Signup(props) {
  ...
  const handleSubmit = (event) => {
    ...
    if(username.length < 8){
      alert("Username should contain at least 8 characters....")
    }else if(password.length < 8){
      alert("Password should contains at least 8 characters...")
    }else if(password != confirmPassword){
      alert("Password is not same as confirm password ...");
    }
    else if(City.length == 0 || Country.length == 0 || address.length == 0 || mobilenumber.length == 0){
      alert("Fields should not be empty.... ")
    }else{
    if(password===confirmPassword)
    {
      console.log("yes");
      fetch("http://34.131.134.106/registor",{
        method:"POST",
        crossDomain:true,
        headers:{
    ...
    <div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className={classes.row}>
          ...
          <div className={classes.enter}><input  placeholder='min 8 characters' className={classes.inp}
              name="username"
              type="text"
              value={username}
        ...
        <div className={classes.enter}><input  className={classes.inp}  placeholder='min 8 characters...'
              name="password"
              type="password"
              value={password}
